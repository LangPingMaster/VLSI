...
always_comb begin : ALU_SIM
    ...
    // Defaults: avoid inferred-latch / incomplete-assignment lint issues
    result_o = 32'd0;
    mul_ss   = 64'sd0;
    mul_uu   = 64'd0;
    ...
    // RV32B unary count instructions: strict decode required by HW table
    else if (opcode == 7'b0010011 &&
             funct7 == 7'b0110000 &&
             funct3 == 3'b001) begin
        case (rs2_for_count)
            5'b00000 : result_o = count_leading_zeros(rs1_date);   // CLZ
            5'b00001 : result_o = count_trailing_zeros(rs1_date);  // CTZ
            5'b00010 : result_o = count_ones(rs1_date);            // CPOP
            default  : result_o = 32'd0;
        endcase
    end
    // DSP custom extension: strict opcode/funct7 decode
        ...
        case (funct3)
           ...
           3'b011 : result_o = (rs2_for_count == 5'b00000) ? ABS(rs1_date)   : 32'd0;
           3'b100 : result_o = (rs2_for_count == 5'b00000) ? CLIP8(rs1_date) : 32'd0;
           default: result_o = 32'd0;
        endcase
    end
    else begin
        result_o = 32'd0; // unsupported/illegal encoding
    end
